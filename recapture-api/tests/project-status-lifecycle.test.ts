...
async function createJobFor(projectId: string, idempotencyKey?: string) {
  const req = request(app)
    .post('/jobs')
    .set(auth)
    .send({ projectId, objectSize: 'medium', expectedFilesCount: 73 });
  if (idempotencyKey) req.set('Idempotency-Key', idempotencyKey);
  return req;
}

/** Valid MEDIUM manifest: 24 photos on each of EYE/TOP/LOW. */
function validManifestBody(): string {
  const photos = ['EYE', 'TOP', 'LOW'].flatMap((ring) =>
    Array.from({ length: 24 }, (_, i) => ({ photoId: `${ring}_${i}`, ringName: ring }))
  );
...
function mockS3(objectCount = 73) {
