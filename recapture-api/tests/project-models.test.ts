...
describe('GET /projects/:id/models — every model the owner has', () => {
  it('lists all SUCCEEDED models newest-first, skipping the ones with nothing to open', async () => {
    const owner = await makeUser('USER');
    const artist = await makeUser('MODEL_ARTIST');
    const project = await makeProject(owner.id);
    const { job } = await makeFinalizedJob(owner.id, project.id as string);
    ...
    const older = await ProjectModel.create({
      ...
      artifacts: {
        glbKey: 'k/older.glb',
        cdnUrls: { glb: 'https://test.cloudfront.net/k/older.glb' },
      },
    });
    const newer = await ProjectModel.create({
      ...base,
      ...
      artifacts: {
        glbKey: 'k/newer.glb',
        cdnUrls: { glb: 'https://test.cloudfront.net/k/newer.glb' },
      },
    });
    // Neither of these may appear: a failure has nothing to open (and carries
    // staff copy), a queued run has no artifacts at all.
    await ProjectModel.create({ ...base, status: 'FAILED' });
    await ProjectModel.create({ ...base, status: 'QUEUED' });

    const res = await request(app).get(`/projects/${project.id}/models`).set(owner.auth);

    expect(res.status).toBe(200);
    expect(res.body.models.map((m: { id: string }) => m.id)).toEqual([newer.id, older.id]);
    expect(res.body.models[0].glbUrl).toBe('https://test.cloudfront.net/k/newer.glb');
    expect(res.body.models[0].isAutoGenerated).toBe(true);
    expect(res.body.models[1].isAutoGenerated).toBe(false);

    // Same owner-safety contract as the single-model projection: no key layout,
    // no selection, no upstream task id, no staff actor.
    const body = JSON.stringify(res.body);
    expect(body).not.toContain('meshy-task-secret');
    expect(body).not.toContain('selectedKeys');
    expect(body).not.toContain('glbKey');
    expect(body).not.toContain(artist.id);
  });

  it('expands ONE optimized generation into both builds — the served one first', async () => {
    // The regression this covers: a capture produces a model and the pipeline
    // then optimizes it. That is ONE ProjectModel record with two renditions,
    // so an owner told "your model was optimized" could find only the served
    // one and had no way to compare them.
    const owner = await makeUser('USER');
    const project = await makeProject(owner.id);
    const { job } = await makeFinalizedJob(owner.id, project.id as string);
    ...
    const record = await ProjectModel.create({
      ...
      createdBySystem: true,
      artifacts: {
        glbKey: 'k/original.glb',
        cdnUrls: { glb: 'https://test.cloudfront.net/k/original.glb' },
      },
      optimized: {
        status: 'SUCCEEDED',
        pipelineVersion: 2,
        // A validated run auto-promotes to 'web' — so the OPTIMIZED build is
        // the one being served, and the original is the hidden alternative.
        activeVariant: 'web',
        manifest: {
          modelId: 'm',
          pipelineVersion: 2,
          ...
          variants: [
            {
              id: 'original',
              url: 'https://test.cloudfront.net/k/original.glb',
              key: 'k/original.glb',
              bytes: 7_920_000,
              triangles: 200_000,
              textureCount: 3,
              largestTextureBytes: 4_000_000,
              meshoptCompressed: false,
            },
            {
              id: 'web',
              url: 'https://test.cloudfront.net/k/v2/web.glb',
              key: 'k/v2/web.glb',
              bytes: 318_000,
              triangles: 60_000,
              textureCount: 3,
              largestTextureBytes: 180_000,
          ...
          physicalSize: {
            ...
            heightMeters: 0.4,
            depthMeters: 0.3,
            longestDimMeters: 0.4,
          },
          reduction: {
            bytesBefore: 7_920_000,
            bytesAfter: 318_000,
            ratio: 0.04,
            trianglesBefore: 200_000,
            trianglesAfter: 60_000,
          },
        },
      },
    });
    ...
    // Served first, then the alternative. Same id — one generation, one charge.
    const [served, alternative] = res.body.models;
    expect(served.variant).toBe('web');
    expect(served.isActiveVariant).toBe(true);
    expect(served.glbUrl).toBe('https://test.cloudfront.net/k/v2/web.glb');
    expect(served.metrics.bytes).toBe(318_000);

    expect(alternative.variant).toBe('original');
    expect(alternative.isActiveVariant).toBe(false);
    // The whole point: a DIFFERENT file, not the served one relabelled.
    expect(alternative.glbUrl).toBe('https://test.cloudfront.net/k/original.glb');
    expect(alternative.metrics.bytes).toBe(7_920_000);

    expect(served.id).toBe(record.id);
    expect(alternative.id).toBe(record.id);
  });

  it('stays a single entry when there is nothing to compare against', async () => {
    // Optimization that FAILED (or never ran) produced no second file — a row
    // for it would point at the original under an "optimized" label.
    const owner = await makeUser('USER');
    const project = await makeProject(owner.id);
    const { job } = await makeFinalizedJob(owner.id, project.id as string);
    ...
    await ProjectModel.create({
      ...
      optimized: {
        ...
        error: { code: 'PIPELINE_FAILED', message: 'internal detail' },
      },
    });
    ...
    // The pipeline's failure detail is staff-facing and must not ride along.
    expect(JSON.stringify(res.body)).not.toContain('internal detail');
  });

  it('is empty for a project with no finished model, and 404s for a stranger', async () => {
    const owner = await makeUser('USER');
    const project = await makeProject(owner.id);

    const mine = await request(app).get(`/projects/${project.id}/models`).set(owner.auth);
    expect(mine.status).toBe(200);
    expect(mine.body.models).toEqual([]);

    // Not-owned answers exactly like missing — this route must not become a way
    // to probe for someone else's project.
    const stranger = await makeUser('USER');
    const theirs = await request(app).get(`/projects/${project.id}/models`).set(stranger.auth);
    expect(theirs.status).toBe(404);
  });
});
