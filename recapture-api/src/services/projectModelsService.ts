...
export interface ProjectModelDto {
  ...
  /**
   * The web-optimization result and its before/after numbers. Absent until the
   * ASSET_OPTIMIZATION job has run (and on every record predating it).
   *
   * On the STAFF DTO in full — including the audit report key and the failure
   * reason — because this is the surface an admin uses to compare the original
   * against the optimized variant and decide which one to serve.
   */
   ...
   * original and the web-optimized build — so an artist can see and compare
   * both. They deliberately share the same `id`: there is still exactly ONE
   * ProjectModel record and ONE paid generation behind them, which is what
   * keeps `approve`, the variant PATCH, and the 24h spend ceiling correct.
   * Never treat two entries with the same id as two models.
   */
  variant: AssetVariantId;
  /** True when this is the rendition owners are currently served. */
  isActiveVariant: boolean;
  /**
   * Per-variant weight, so the two entries can be compared without opening a
   * 3D tool. Absent until the optimization pipeline has measured them.
   */
  metrics?: {
    bytes: number;
    triangles: number;
...
export interface OwnerModelDto {
  ...
  /**
   * The untouched Meshy GLB, always present. `glbUrl` above is whichever
   * variant an admin promoted — these two are equal until someone promotes the
   * optimized one.
   */
  originalGlbUrl: string;
  /** The web-optimized variant, when one has been produced and passed its gates. */
...
export function toProjectModelDto(record: IProjectModel): ProjectModelDto {
  return {
    ...
    variant: 'original',
    isActiveVariant: record.optimized?.activeVariant !== 'web',
    ...(variantMetrics(record, 'original') ?? {}),
...
/**
 * Flattens the Mongoose sub-document into plain JSON.
 *
 * `manifest` is stored as Mixed, so it comes back as a POJO already; the
 * round-trip through JSON here is what guarantees no Mongoose internals (or a
 * stray `_id`) leak into a response that clients cache.
 */
function flattenOptimized(optimized: OptimizedAsset): OptimizedAsset {
  return JSON.parse(JSON.stringify(optimized)) as OptimizedAsset;
}

/**
 * Pulls one variant's measurements out of the stored manifest, shaped for
 * spreading into a DTO. Returns undefined (not zeros) when the pipeline has not
 * measured this model — a missing number must read as "unknown", never as "0
 * bytes", which would show as a spectacular fake saving.
 */
function variantMetrics(
  record: IProjectModel,
  variant: AssetVariantId
): { metrics: ProjectModelDto['metrics'] } | undefined {
  const found = record.optimized?.manifest?.variants.find((v) => v.id === variant);
  if (!found) return undefined;
  return {
    metrics: {
      bytes: found.bytes,
      triangles: found.triangles,
      textureCount: found.textureCount,
      largestTextureBytes: found.largestTextureBytes,
    },
  };
}

/**
 * Expands one generation record into the entries the staff list shows: the
 * Meshy original, plus the web-optimized build once it exists.
 *
 * WHY EXPANSION AND NOT A SECOND RECORD: `ProjectModel` is one row per PAID
 * generation, and `countServerSelectedGenerationsInLast24h` counts rows. A
 * second row per optimization would double-count Meshy spend against the daily
 * ceiling and make the cost audit lie. So the second entry is a VIEW of the
 * same record — same `id`, different `variant` — and every id-keyed action
 * (approve, promote) keeps working unchanged.
 *
 * Only a SUCCEEDED optimization expands. A skipped one produced no second file,
 * and a failed one has nothing to compare against.
 */
export function toProjectModelDtos(record: IProjectModel): ProjectModelDto[] {
  const original = toProjectModelDto(record);

  const web = record.optimized?.manifest?.variants.find((v) => v.id === 'web');
  if (record.optimized?.status !== 'SUCCEEDED' || !web) return [original];
  ...
  return [
    original,
    {
      ...
      variant: 'web',
      isActiveVariant: record.optimized.activeVariant === 'web',
      // The optimized GLB, with the poster and USDZ carried over: those are not
      // re-derived by the pipeline, so both entries legitimately share them.
      artifacts: {
        ...(original.artifacts ?? { glb: web.url }),
        glb: web.url,
      },
      ...(variantMetrics(record, 'web') ?? {}),
    },
  ];
}
...
function toOwnerModelDto(record: IProjectModel): OwnerModelDto | null {
  ...
  return {
    ...
    // Both URLs, so a client (or a reviewer on a real device) can flip between
...
// ── Variant promotion (the admin decision) ───────────────────────────────────

export type SetActiveVariantResult =
  | { outcome: 'MODEL_NOT_FOUND' }
  /** No optimization has run for this model, so there is nothing to switch to. */
  | { outcome: 'NOT_OPTIMIZED' }
  /** The requested variant was never produced (or failed its gates). */
  | { outcome: 'VARIANT_UNAVAILABLE'; variant: AssetVariantId }
  | { outcome: 'UPDATED'; model: IProjectModel };
 ...
 * at a 404.
 */
export async function setActiveModelVariant(
  projectId: string,
  modelId: string,
  variant: AssetVariantId
): Promise<SetActiveVariantResult> {
  const record = await ProjectModel.findOne({
    _id: new Types.ObjectId(modelId),
    projectId: new Types.ObjectId(projectId),
  }).exec();
  if (!record) return { outcome: 'MODEL_NOT_FOUND' };

  // 'original' is always available — it is the re-hosted Meshy GLB itself, and
  // reverting must work even when optimization never ran or failed outright.
  if (variant !== 'original') {
    if (!record.optimized) return { outcome: 'NOT_OPTIMIZED' };
    const available = record.optimized.manifest?.variants.some((v) => v.id === variant);
    if (!available) return { outcome: 'VARIANT_UNAVAILABLE', variant };
  }

  if (!record.optimized) {
    // Reverting a never-optimized record is a no-op, not an error.
